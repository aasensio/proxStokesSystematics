...
def plotSingleIUWT(data, data2, row, valueL1):
    ...
    ax[row,1].set_xlim([10825.8,10833])
...
# IUWT
lambdaL1 = [3e-3,7e-3,1e-2,3e-2]
dat = []
for l in lambdaL1:
    out = np.load("results/hazel_iuwt_lambda_{0}_inner_100.npz".format(l))
    dat.append(out)

dat2 = []
for l in lambdaL1:
    out = np.load("results/hazel_iuwt_lambda_{0}_inner_1.npz".format(l))
    dat2.append(out)
...
pl.tight_layout()
pl.savefig('figures/hazel_iuwt.pdf')
...
ax[-1,0].set_xlabel(r'Wavelength [$\AA$]')
ax[-1,1].set_xlabel(r'Index')

pl.tight_layout()
pl.savefig('figures/hazel_wavelet.pdf')

# f, ax = pl.subplots(nrows=1, ncols=3, figsize=(12,5))
# for i in range(len(lambdaL1)):
#     ax[0].semilogy(dat[i]['arr_5'], label="$\lambda=${0}".format(lambdaL1[i]*1e-3))
#     ax[1].plot(dat[i]['arr_12'])
#     ax[2].semilogy(dat[i]['arr_5']+lambdaL1[i] * dat[i]['arr_12'])

# ax[0].set_xlabel('Iteration')
# ax[1].set_xlabel('Iteration')
# ax[2].set_xlabel('Iteration')
# ax[0].set_ylabel('$\chi^2$')
# ax[1].set_ylabel(r'$\vert \alpha \vert_0$')
# ax[2].set_ylabel(r'$\chi^2 + \lambda \vert \alpha \vert_0$')

# ax[0].legend()
# pl.tight_layout()
# pl.savefig('figures/convergence_hazel.pdf')